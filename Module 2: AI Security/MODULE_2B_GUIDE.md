...
5. **USE AI FUNCTIONS ON ACCOUNT** (mentionné en note dans le script, pas démontré) : Privilège de niveau COMPTE, accordé à PUBLIC par défaut. Fait double emploi avec les database roles ci-dessus — les deux sont requis pour appeler des fonctions AI. En pratique, les database roles suffisent pour le contrôle granulaire. USE AI FUNCTIONS sert de kill-switch global si nécessaire. Mentionner uniquement si la question est posée.
...
### Transition vers 2C
« On a les contrôles au niveau modèle (Model RBAC) et au niveau feature (database roles). Maintenant on passe aux contrôles probabilistes — AI_REDACT pour la protection PII dans le texte libre. »

### Certification
- **D4.2** : Model allowlist, application roles, feature access
