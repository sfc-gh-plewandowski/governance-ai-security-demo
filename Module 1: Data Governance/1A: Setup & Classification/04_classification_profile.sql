...
-- ════════════════════════════════════════════════════════════
-- PARTIE J : COMBLER LES LACUNES DE LA CLASSIFICATION
-- ════════════════════════════════════════════════════════════
-- La classification automatique excelle sur les STRING (regex).
-- Pour les colonnes DATE et NUMBER, elle ne détecte pas toujours
-- les catégories sémantiques (DATE_OF_BIRTH, SALARY, etc.).
--
-- On complète manuellement avec les MÊMES tags que le profil
-- aurait posés si la détection avait fonctionné.
-- Le tag_map du profil liste SENSIBILITE = 'SENSIBLE' pour
-- DATE_OF_BIRTH et SENSIBILITE = 'INTERNE' pour SALARY.

-- Dates de naissance (DATE) — non détectées
ALTER TABLE VOLTAIRE_RH.EMPLOYES.PERSONNEL MODIFY COLUMN
    DATE_NAISSANCE SET TAG VOLTAIRE_GOVERNANCE.TAGS.SENSIBILITE = 'SENSIBLE';
ALTER TABLE VOLTAIRE_CRM.CONTACTS.PERSONNES MODIFY COLUMN
    DATE_NAISSANCE SET TAG VOLTAIRE_GOVERNANCE.TAGS.SENSIBILITE = 'SENSIBLE';
ALTER TABLE VOLTAIRE_RH.RECRUTEMENT.CANDIDATS MODIFY COLUMN
    DATE_NAISSANCE SET TAG VOLTAIRE_GOVERNANCE.TAGS.SENSIBILITE = 'SENSIBLE';

-- Permis de conduire (STRING) — classifié DRIVERS_LICENSE mais pas tagué SENSIBILITE
ALTER TABLE VOLTAIRE_RH.EMPLOYES.PERSONNEL MODIFY COLUMN
    PERMIS_CONDUIRE SET TAG VOLTAIRE_GOVERNANCE.TAGS.SENSIBILITE = 'SENSIBLE';

-- Salaires (NUMBER) — non détectés
ALTER TABLE VOLTAIRE_RH.PAIE.BULLETINS MODIFY COLUMN
    SALAIRE_BRUT SET TAG VOLTAIRE_GOVERNANCE.TAGS.SENSIBILITE = 'INTERNE';
ALTER TABLE VOLTAIRE_RH.PAIE.BULLETINS MODIFY COLUMN
    SALAIRE_NET SET TAG VOLTAIRE_GOVERNANCE.TAGS.SENSIBILITE = 'INTERNE';
ALTER TABLE VOLTAIRE_RH.RECRUTEMENT.CANDIDATS MODIFY COLUMN
    SALAIRE_SOUHAITE SET TAG VOLTAIRE_GOVERNANCE.TAGS.SENSIBILITE = 'INTERNE';

-- Montants financiers (NUMBER) — non détectés
ALTER TABLE VOLTAIRE_FINANCE.COMPTABILITE.FACTURES MODIFY COLUMN
    MONTANT_HT SET TAG VOLTAIRE_GOVERNANCE.TAGS.SENSIBILITE = 'INTERNE';
ALTER TABLE VOLTAIRE_FINANCE.COMPTABILITE.FACTURES MODIFY COLUMN
    MONTANT_TTC SET TAG VOLTAIRE_GOVERNANCE.TAGS.SENSIBILITE = 'INTERNE';
ALTER TABLE VOLTAIRE_FINANCE.TRESORERIE.TRANSACTIONS MODIFY COLUMN
    MONTANT SET TAG VOLTAIRE_GOVERNANCE.TAGS.SENSIBILITE = 'INTERNE';

-- ════════════════════════════════════════════════════════════
-- PARTIE K : BILAN FINAL CROSS-DATABASE
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- PARTIE L : ACTIVER L'AUTO-CLASSIFICATION SUR LES BASES
-- ════════════════════════════════════════════════════════════
